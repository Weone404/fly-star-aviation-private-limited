...
${text(PAD, 82, title, { size: 34, weight: 700 })}
${body}
...
const render = {
  ...
  timeline({ unit, max, bars }) {
    ...
    const top = 168, gap = 96;
    let out = text(PAD, 118, unit, { size: 18, fill: P.muted });
    for (let t = 0; t <= max; t++) {
