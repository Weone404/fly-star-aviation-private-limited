 ...
 */
/**
 * Diagram covers, which replace the decorative cover for the posts that have
 * one.
 *
 * The illustrations below were built to a house style that forbade text,
 * letters and numbers in an image, because generated lettering is unreliable
 * and a wrong figure on a page whose whole claim is checked figures is worse
 * than no picture. That reasoning holds. The conclusion did not: the answer is
 * a picture whose text is typed rather than generated.
 *
 * These SVGs are rendered by scripts/generate-blog-diagrams.mjs from
 * src/blogDiagrams data that is copied out of the post itself, so a diagram
 * cannot say something the article does not. Real text, a few kilobytes, crisp
 * at any width, and it carries the source line in the picture.
 */
const DIAGRAM_COVERS = {
  'dgca-exam-subjects-by-licence': {
    alt: 'Table comparing PPL, CPL and ATPL: computer number required for all three, qualification to appear, 70 per cent pass mark per paper, and paper validity of two and a half years for PPL against five years for CPL and ATPL.',
    caption: 'What each licence requires to sit the theory papers. Source: CAR Section 7, Series B, Part I.',
  },
  'dgca-exam-attempts-and-validity': {
    alt: 'Bar chart of how long a passed DGCA paper stays valid: PPL two and a half years, CPL five years, ATPL five years.',
    caption: 'Validity runs from the date of passing. Source: CAR Section 7, Series B, Part I.',
  },
  'cpl-eligibility-after-12th': {
    alt: 'Five numbered steps before a CPL: 10+2 with Physics and Mathematics, Class 1 medical, DGCA computer number, theory papers at 70 per cent, then flying hours at an approved FTO.',
    caption: 'The order matters: the medical is the cheapest step that can end the plan.',
  },
  'atpl-eligibility-india': {
    alt: 'Table contrasting CPL and ATPL on prerequisite licence, typical career stage, number of theory subject areas and flying experience.',
    caption: 'An ATPL is not a first licence. Source: CAR Section 7, Series B, Part I.',
  },
  'dgca-olode-vs-regular-exams': {
    alt: 'Table comparing regular DGCA sessions with Online On-Demand sessions: four against eight sessions in 2026, five against three days, identical eligibility and pass mark.',
    caption: 'Same exam, different schedule. Source: DGCA Programme of Examinations 2026.',
  },
  'dgca-exam-pass-rate': {
    alt: 'Table of pass-rate claims against what backs each: no DGCA document found for a national pass percentage or per-paper rates; the 70 per cent pass mark itself is in CAR Section 7.',
    caption: 'What is published, and what is only repeated.',
  },
  'how-to-choose-a-flying-school-in-india': {
    alt: 'Bar chart of the DGCA FTO ranking weights: operational aspects 40 per cent, FTO performance 20, safety standards 20, compliance 10, assistance to students 10.',
    caption: 'Operational ratios carry the largest single block. Source: DGCA Public Notice F. No: DGCA-16017/10/2025-DFT.',
  },
...
export function imagesFor(slug) {
  const planned = (slug && BLOG_IMAGES[slug]) || []
  const diagram = slug && DIAGRAM_COVERS[slug]
  if (!diagram) return planned

  const cover = {
    slot: 'cover',
    file: `/blog/diagrams/${slug}.svg`,
    ready: true,
    alt: diagram.alt,
    caption: diagram.caption,
    prompt: 'Generated by scripts/generate-blog-diagrams.mjs from the post\'s own sourced facts.',
  }
  return [cover, ...planned.filter((img) => img.slot !== 'cover')]
}
